...
namespace UITools
{
    ...
    public static class ConfigurationMenu
    {
        ...
        /// </param>
        public static void Add(string title, (string name, Func<Transform, GameObject> createWindowFunc)[] buttons)
        {
            window.AddModeButtons(title, buttons);
        }
    }
    ...
    class ConfigurationWindow
    {
        ...
        public void Initialize(int width, int height, int posX, int posY, string title,
            Builder.SceneToAttach attachMode)
        {
            if (initialized)
                return;
            initialized = true;
        ...
        {
            Object.Destroy(currentDataScreen);
            currentDataScreen = createWindowFunc.Invoke(mainWindow) ??
                                Builder.CreateBox(mainWindow, RecommendedContentSize.x, RecommendedContentSize.y)
                                    .gameObject;
        ...
        void AddMenu(string name, Func<Transform, GameObject> createWindowFunc)
        {
            ...
            Button selectButton = Builder.CreateButton(categoriesButtonsBox, 180, 60, 0, 0,
                () => SetScreen(createWindowFunc, index), name);
        ...
        public void AddModeButtons(string title, (string name, Func<Transform, GameObject> createWindowFunc)[] buttons)
        {
            if (categoriesButtons.Count > 0)
                AddSeparator();
            AddTitle(title);
            foreach ((string name, Func<Transform, GameObject> createWindowFunc) element in buttons)
