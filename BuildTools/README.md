...
- BT-014 已安装：JSON 页有独立字号 ± 按钮，旧默认 20 首次迁移到 28；三个切页按钮固定在浮窗顶部，右下角 `//` 可拖动调整尺寸。Part 页对单选 Fuel Tank 显示燃料百分比，对单选 Engine Titan 显示已有的三个开关，其余原始字段继续保留。用户已验收字号调整、拖角缩放与切页保持、收起/展开、短窗口 Build 滚动、JSON 放大后的排版与未选零件提示，以及 Engine Titan 开关。多选规则仍在 [设计记录](notes/2026-09-23-multi-part-editor-design.md) 中讨论，尚未启用。
...
- `bin/Release/BuildTools.dll`：当前 BT-014 构建产物，本机新增界面已由用户验收。
- `PROJECT_STATE.md`、`PROJECT_LOG.md`：本项目的状态和已验证记录。
- `outputs/BuildTools-Windows-handoff-BT-014.zip`：Build Tools + 前置 UITools 的 Windows 手动交接包；包含两个 DLL、两份项目源码、说明及校验值，不含自动安装器。若需要同时交付 Workspace 0.4.0，使用 `../BlueprintWorkspace/outputs/SFS-Windows-Manual-Handoff-0.4.0-BT-014-2026-09-23.zip`。旧包保留作回退基线。
